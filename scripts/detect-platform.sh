#!/usr/bin/env bash
# Probe host and write .pdf2md/platform.json for one of three profiles:
#   mac_arm, linux_cpu, linux_gpu
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: detect-platform.sh [--cpu] [--redetect] [--yes]
...
# Map driver CUDA version to nearest PyTorch wheel tag.
