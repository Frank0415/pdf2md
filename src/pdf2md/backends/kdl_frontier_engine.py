"""KDL-Frontier-Parser-nano engine (vendored from ParseBench kdl_frontier_nano provider).

Requires a vLLM OpenAI-compatible endpoint serving KDLAI/KDL-Frontier-Parser-nano:
...
Set KDL_NANO_ENDPOINT_URL (e.g. http://localhost:8000/v1) before converting.
"""
import asyncio
import base64
import enum
...
class KdlFrontierConfigError(RuntimeError):
    """Missing or invalid KDL Frontier configuration."""


class KdlFrontierError(RuntimeError):
    """KDL Frontier conversion failed."""
...
def load_page_images(pdf_path: Path, *, dpi: int | None = None, max_pages: int | None = None) -> list[Image.Image]:
    pdf_path = Path(pdf_path).resolve()
    if pdf_path.suffix.lower() in (".png", ".jpg", ".jpeg", ".jfif"):
        return [Image.open(pdf_path)]

    dpi = int(dpi or os.getenv("KDL_NANO_DPI", "144"))
    max_pages = int(max_pages or os.getenv("KDL_NANO_MAX_PAGES", "400"))
    ...
    with fitz.open(str(pdf_path)) as doc:
        if doc.page_count > max_pages:
            raise KdlFrontierError(
                f"Document has {doc.page_count} pages > max_pages={max_pages}."
            )
        for page in doc:
...
def parse_document(
    ...
    *,
    ...
    endpoint_url = (endpoint_url or os.getenv("KDL_NANO_ENDPOINT_URL") or "").rstrip("/")
    if not endpoint_url:
        raise KdlFrontierConfigError(
            "KDL_NANO_ENDPOINT_URL is required (vLLM OpenAI-compatible base URL ending in /v1)."
        )
    model = model or os.getenv("KDL_NANO_MODEL", "kdl-frontier-parser-nano")
    max_concurrent = int(max_concurrent or os.getenv("KDL_NANO_MAX_CONCURRENT", "8"))

    engine = _NanoEngine(endpoint_url, model, max_concurrent, timeout_s)
    result = _run_async(engine.parse_pages(page_images))
    markdown = result.get("markdown")
    if not markdown:
        raise KdlFrontierError("KDL Frontier produced empty markdown.")
    return str(markdown)
